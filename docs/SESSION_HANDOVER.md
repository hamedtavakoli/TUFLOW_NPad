...
- `npm test` failed because PowerShell blocked `npm.ps1` execution.
- `npm.cmd test` passed:
  - 17 test files passed.
  - 127 tests passed.
- `npm.cmd run build` failed during TypeScript compilation because CodeMirror modules could not be resolved:
  - `@codemirror/autocomplete`
  - `@codemirror/commands`
  - `@codemirror/state`
  - `@codemirror/view`
- `npm.cmd ls @codemirror/autocomplete @codemirror/commands @codemirror/state @codemirror/view` returned `(empty)`, so the build failure appears related to missing local installed dependencies rather than an obvious source-code regression.
...
- Production build currently fails locally until missing CodeMirror dependencies are restored in `node_modules`.
...
- Compare logic is custom LCS-based and covered by tests, but TODO notes suggest it needs more modelling-user review.
...
Fix local dependency/build health first.

1. Run `npm install` or `npm ci` to restore CodeMirror dependencies in `node_modules`.
2. Re-run `npm.cmd ls @codemirror/autocomplete @codemirror/commands @codemirror/state @codemirror/view`.
3. Re-run `npm.cmd test`.
4. Re-run `npm.cmd run build`.

After build health is restored, the best feature task is to preserve reference-check results per tab or avoid clearing reference-check visibility/status on tab switches. This directly matches the TODO note and improves a core workflow without inventing a new feature.
